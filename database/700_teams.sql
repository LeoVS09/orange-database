...
create table app_public.teams_profiles(
...
alter table app_public.teams_profiles enable row level security;

create trigger _100_timestamps
  after insert or update on app_public.teams_profiles
  for each row
  execute procedure app_private.tg__update_timestamps();

------------------------------------------------------------------------------------------------------------------------

create policy select_all   on app_public.teams_profiles for select using (true);
create policy insert_teacher on app_public.teams_profiles for insert with check (app_public.current_user_is_teacher());
create policy update_teacher on app_public.teams_profiles for update using (app_public.current_user_is_teacher());
create policy delete_teacher on app_public.teams_profiles for delete using (app_public.current_user_is_teacher());

------------------------------------------------------------------------------------------------------------------------

grant select       on app_public.teams_profiles to orange_visitor;
grant insert(team_id, profile_id) on app_public.teams_profiles to orange_visitor;
grant delete       on app_public.teams_profiles to orange_visitor;
